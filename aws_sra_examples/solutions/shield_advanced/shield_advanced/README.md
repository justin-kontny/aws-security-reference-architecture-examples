...
- shield is enabled from the delegated administrator account.
- EC2, ECR, Lambda standard and Lambda code scanning is enabled.
...
1. Log into the `management account` and navigate to the shield page
   1. Select Settings and then General
   1. Verify that the delegated admin account is set for each region
2. Log into the Audit account and navigate to the shield page
   1. Verify the shield service is enabled in each region
   ...
   3. Verify all existing member accounts have shield ec2, ecr, and lambda standard scanning enabled in each region, and lambda code scanning in supported regions
3. Log into a member account and verify the shield is enabled and configured to scan ec2, ecr, lambda functions and lambda code
...
1. In the `management account (home region)`, delete the AWS CloudFormation **Stack** (`sra-shield-advanced-main-ssm` or `sra-shield-advanced-main`).
2. In the `management account (home region)`, delete stack instances from the the AWS CloudFormation **StackSet** (`sra-shield-advanced-main-ssm` or `sra-shield-advanced-main`).
...
- [Managing multiple accounts in Amazon shield with AWS Organizations](https://docs.aws.amazon.com/shield/latest/user/managing-multiple-accounts.html)
